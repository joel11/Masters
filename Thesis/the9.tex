...
\begin{document} 
	...
	\begin{itemize}
%		\item [1] The nominal expected return for $t+D$ from all stocks traded at $t$
%		\begin{equation}
	%	\mathbf{\hat{O}}_{t+D} = \sum_{s \in stocks} \mathbf{E[r]}_{s, {t+D}}
	%	\end{equation}
	%	\item [2] The nominal observed return for $t+D$ from all stocks traded at $t$
	%	\begin{equation}
%		\mathbf{\tilde{O}}_{t+D} = \sum_{s \in stocks} \mathbf{\tilde{r}}_{s, {t+D}}
%		\end{equation}
	%	\item [3] The nominal benchmark return for $t+D$ from all assets traded at $t$
%		\begin{equation}
%		\mathbf{O}^b_{t+D} = \sum_{s \in assets} \mathbf{r}^b_{s, {t+D}}
%		\end{equation}
		...
		\begin{equation}
		\mathbf{\tilde{r}}_{t + D} = \frac{\left( \mathbf{\tilde{p}}_{t+D} - \mathbf{f}_t \right) }
		{\mathbf{f}_t}* \mathbf{s}_t
		\end{equation}	
		\item [12] The fullcost expected return rate at $t+D$
		\begin{equation}
		\mathbf{\hat{r}_{t + D}} = \frac{\left( \mathbf{\hat{p}}_{t+D} - \mathbf{f}_t \right) }
		{\mathbf{f}_t}* \mathbf{s}_t
		\end{equation}
		\item [13] The fullcost benchmark return rate at $t+D$
		\begin{equation}
		\mathbf{r}^b_{t + D} = \frac{\left( \mathbf{\tilde{p}}_{t+D} - \mathbf{f}^b_t \right) }
		{\mathbf{f}^b_t}* \mathbf{s}^b_t
				\end{equation}	
		
		\item [8] The Sharpe Ratio
		\begin{equation}
			\mathbf{SR} = \mathbf{\hat{\tilde{R}}_{{t+D}}}
		\end{equation}
	\end{itemize}
	\hfill\break
