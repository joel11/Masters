...
%	cular price trend, or is in a particular state \citep{Murphy, LoHeretics}.  
...
%	successful \citep{Shalev, Zhang}. 
	...
	\begin{itemize}
		...
		\item[2] The weights from the $j^{th}$ node in the $\ell^{th}$ layer from the $k^{th}$ node in the $(\ell-1)^{th}$ layer are represented by $\mathbf{w}^\ell_{jk}$
		\item[3] The bias for the $j^{th}$ node in layer $\ell$ is represented by $\mathbf{w}^\ell_0j$
		\item[4] The output for the $j^{th}$ node is defined as 
		\begin{equation}\label{eq_activation_output}
		\mathbf{a} = \theta(\mathbf{z}_j) ,
		\end{equation} 
	...
	\theta^\prime(x) = \mathrm{o}(x)(1-\mathrm{o}(x))
	\end{equation}
	...
	\begin{itemize}
		\item [1] \textbf{Forward Pass}: The samples are propagated through the network, in order to generate the output $\hat{y}_s$. Vectorization is typically used to calculate the results for multiple samples at once, though conceptually each sample is used as input for the first layer, and subsequent layers use the weighted output from the previous layer passed through the activation function as input, using equation \ref{eq_weighted_input} for the weighted output and the equations defined in \ref{imp_activation_functions} for the activation functions.
		\newline
		\item [2] \textbf{Calculate Cost}: The cost between the training output $y_s$ and the model output $\hat{y}_s$ is calculated. Using this cost function, the rate of change for the error in the output layer by the rate of change in the activation function in the last layer. This rate of change for the error is expressed as:	
		...
		\mathbf{\lambda}^L = (\mathbf{a}^L - \mathbf{y}) \otimes \theta^{\prime}(\mathbf{z}^L) .
		\end{equation}
		\newline		
		...
		\begin{itemize}
			\item [3.1] The activation values are propagated back through the network to calculate the delta values based on the cost value. This uses the same rationale as equation \ref{eq_lambda1}, but rather uses $(w^{\ell+1})^T$ by the next layers error in order to get an indication of how the error rate and how it moves backwards through the network.
			\begin{equation}\label{eq_lambda4}
			\mathbf{\lambda}^\ell = ((\mathbf{w}^{\ell+1})^T\mathbf{\lambda}^{\ell+1})  \otimes \theta^{\prime}(\mathbf{z}^\ell) .
			\end{equation}
			...
			\mathbf{w}^\ell \rightarrow \mathbf{w}^\ell - {\eta}\mathbf{\lambda}^{\ell} (\mathbf{a}^{\ell - 1})^T ,
			\end{equation}
	...
	\RestyleAlgo{algoruled}
	\begin{algorithm}[H]
		\KwIn{Neural Network $N$, with randomly initialized weights $\mathbf{w}$, $L$ layers, and activation functions $\theta$ and learning rate $\eta$}
	...
	\begin{itemize}
		...
		\mathbf{w}^\ell \rightarrow \mathbf{w}^\ell - \frac{\eta}{m} \sum_{x} \mathbf{\lambda}^{x, \ell} (\mathbf{a}^{x, \ell - 1})^T , 
		\end{equation}
		where
		\begin{enumerate}
			...
			\item $\lambda$ is the rate of change in the error (calculated using equation \ref{eq_lambda4}),
			\item $a$ is the output of the activation function for the weighted input (calculated using equation \ref{eq_activation_output})
		\end{enumerate}
	\end{itemize}
	...
	\mathbf{C}=\frac{1}{2 n} \sum_{x}\left\|\mathbf{y}-\mathbf{a}^{L}\right\|^{2}+\frac{\mathrm{\Gamma}}{2 n} \sum_{w} \mathbf{w}^{2}
	\end{equation}
	where
	\begin{enumerate}
		\item $n$ is the dataset sample size,
		\item $y$ is the input data,
		\item $a$ is the output of the activation function in the $L^{th}$ layer (calculated using equation \ref{eq_activation_output}),
		\item $\Gamma$ is the configured regularization rate, and
		\item $w$ is a layer's weights.
	\end{enumerate}
	
	The additional term is scaled by the configurable regularization parameter, $\Gamma$, which if small approximates the original cost function or if large will increase the degree of regularization used. \newline
	...
	\mathbf{w} \rightarrow \mathbf{w}-\eta \frac{\partial \mathbf{C}_{0}}{\partial \mathbf{w}}-\frac{\eta \mathrm{\Gamma}}{n} \mathbf{w}
	\end{equation}
	
	For SGD, this can be simplified to the following: 
	
	\begin{equation}\label{func_sgd_l2}
	\mathbf{w} \rightarrow\left(1-\frac{\eta \mathrm{\Gamma}}{n}\right) \mathbf{w}-\frac{\eta}{m} \sum_{x} \frac{\partial \mathbf{C}_{x}}{\partial \mathbf{w}}
	\end{equation}
	...
	The steps for implementing the SAE training, suggested by \citet{Hinton2}, are as follows:
	
	\begin{enumerate}
		\item \textbf{Define Network:}Define a network structure which conforms to the requirements of an SAE, with $L$ layers.
		...
		\begin{enumerate}
			\item Process the data through the previously trained layers using the forward propagation as defined in Section \ref{imp_ffn_functions}.
		...
		\item \textbf{Train Full AutoEncoder:} This network can then be trained using the backpropagation and SGD algorithms, where cost of reproducing the network input is minimised.
	...
	\begin{enumerate}
		\item \textbf{Define Network:} Define a network structure which conforms to the requirements of an SAE, with $L$ layers.
		\item \textbf{Initialize Network Weights:} Use an effective weight initialization for all $L$ layers, such as Xavier or He (discussed fully in Section \ref{imp_weights}).
		\item  \textbf{Train Full AutoEncoder:} This network can then be trained using the backpropagation and SGD algorithms, where cost of reproducing the network input is minimised.
		\item \textbf{Split to Encoder:} Once a minima or acceptable level of reconstruction has been reached, the network can be truncated as the encoder layer, and so the first $L/2$ layers are used as the SAE.
	\end{enumerate}
