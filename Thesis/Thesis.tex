...
The algorithm for implementing the SAE training suggested by Hinton et al \cite{Hinton2} is as follows
...
An adjusted initialization is presented here, dubbed 'DC', where the ReLU initialization uses a mean of the input and output layers in order to scale the weight variance. For networks with constant layer sizes, the intialization is the same as He, though for SAE networks which have layer size changes by definition, the DC initialization will result in more appropriately sized weights.
