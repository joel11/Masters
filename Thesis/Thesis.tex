...
\begin{technicaldoc}
	...
	\paragraph{Critical Implementations}
	\begin{itemize} 
		...
		\item[$\bullet$] Synthetic Tests: Run tests on synthetic data to inform experiment design and parameter choices
	...
	\begin{itemize} 
		...
		\item[$\bullet$] Decide on data splits and what should be used for CSCV
		\item[$\bullet$] Decide on data windows - aribtraty choices or.. ?
	\end{itemize}
...
of trading strategy)} \\\hline
	...
	\begin{itemize}
		\item End to end process: Should the FFN network be trained with SGD prior to OGD, or should the first dataset just be used for OGD training on the FFN after pre-training/weight initialization has taken place?
		\item CSCV data usage: Currently the implementation uses a holdout data set for the CSCV analysis. Though, CSCV shouldn't really require this. I'm unsure in this case whether the entire data set should be run in an online OGD manner to generate returns (after using the same data to train the SAE/FFN networks), or whether it should only use a portion of the data (i.e. what was used during OGD training)
		\item Search technique used: Considering the PBO issues around brute force space exploration, I'm not sure if there are any obviously appropriate search techniques to be used.
		\item Predictions attempted: How many steps and stocks should be attempted in the prediction process?
		\item Rolling Data Windows: These are currently being chosen arbitrarily. Not sure if a more effective method is possible
		\item Trading strategy: The CSCV method requires returns to be used, which can then be assessed with something like the Sharpe Ratio. Should an actual trading strategy be used for each configuration here, or just a representation of accuracy which can be calculated as a single figure for each time period (such as MSE)? If a trading strategy is used, what should it be?
		\item SAE Updating: The current implementation does not have any SAE updates taking place as more data is observed. While easy to change, I'm not sure if or when the SAE network should be updated
		\item Denoising SAE: The initial proposal was to use denoising autoencoders. The implementation process is easy enough, but I'm unsure if denoising would be effective in a financial context (where data is already so noisy)		
		\item Final Dataset: The current dataset doesn't account for survivorship bias, and I'm a little unsure of how best to pick the final stocks/indices etc. (the intial proposal was rather limited in comparison to what is now available)
	\end{itemize}		
...
\begin{itemize}
	\item [$\cdot$] 3 stocks with an upward trend, and high, medium, and low variance levels
	\item [$\cdot$] 3 stocks with a downward trend, and high, medium, and low variance levels
	\item [$\cdot$] 3 stocks with an flat trend, and high, medium, and low variance levels
\end{itemize}
...
\begin{itemize}
	\item [1] The dataset is split into 3 subsets: the SAE/FFN  portion, the OGD portion and the hold-out set for validation
	...
	\item [4] The third and last 'hold-out' subset was used to generate log fluctuation predictions, which can then be used to generate returns from a trading strategy
	\item [5] The returns generated can then be used in the CSCV process, to estimate the probability of backtest overfitting
\end{itemize}
~\\
...
\begin{itemize}
	...
	\item [7] Returns generations
	\begin{itemize}
		\item[$\bullet$] The trading strategy, as per \ref{imp_tradingstrat}, will be used to generate returns from the predicted and actual prices in the holdout dataset. These returns for each time period and each configuration of the model defined by steps 3-5 will form the $M$ matrix for the CSCV process.
	\end{itemize}	
	\item [8] CSCV \& PBO
	\begin{itemize}
		\item[$\bullet$] Using the $M$ matrix from 7, the CSCV process will be run which will then allow a calculation of PBO.
