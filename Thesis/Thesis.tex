...
\begin{technicaldoc}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item[$\cdot$] ReLU + Initialization (done in this version)
	...
	\bigskip\large\textbf{3. Future Status}\bigskip\normalsize
	...
	\begin{itemize} 
		\item[$\bullet$] Training: Regularization methods \& Learning rate annealing schedule are implemented, but need to be tested and incorporated info configuration testing (results so far suggest these will be necessary).
	...
	\begin{itemize} 
		\item[$\bullet$] Finalise Dataset: Decide which data to use - JSE Close / intra day?
	...
	\begin{itemize}
		\item[$\bullet$] Change SAE testing to use K-fold validation		
	...
	\newpage\bigskip\large\textbf{4. Milestones \& Deliverables}\normalsize
	...
	\begin{table}[h]
		\begin{tcolorbox}[tabularx*={\arrayrulewidth0.6mm}{X||X},fontupper=\normalsize\sffamily,colback=white!10!white,colframe=black!40!]
			...
			\centerline {\cellcolor{gray!25}  Done}  & {3 Data Log Fluctuation Processing}  \\\hline
			\centerline {\cellcolor{gray!25}  Done} & {4 SAE Implementation}  \\\hline
			\centerline {\cellcolor{gray!25}  Done} & {5 Online FFN/LSTM Implementation} \\\hline
			\centerline {\cellcolor{gray!25}  Done} & {6 CSCV Implementation} \\\hline		
			\centerline {\cellcolor{gray!25}  Done} & {7 Synthetic Data Generation} \\\hline					
			\centerline {\cellcolor{gray!25}  Done} & {8 Finalise Implementation Process} \\\hline
			\centerline {\cellcolor{gray!25}  Done} & {9 Database Implementation} \\\hline
			\centerline {28/02} & {10 Learning Optimizations} \\\hline			
			\centerline {28/02} & {11 Test full implementation on synthetic
	data} \\\hline
			\vspace{0.1mm}  \centerline {31/03} &\vspace{0.1mm}   {12 Test full implementation on actual
dataset} \\\hline
			\vspace{0.1mm}  \centerline {31/03} &\vspace{0.1mm}   {13 Statistical Analysis (incl. profitability
of trading strategy)} \\\hline
			\vspace{0.1mm}  \centerline {30/04} &\vspace{0.1mm}   {14 Write up \& Revisions} \\\hline
									
		\end{tcolorbox}
	\end{table}
	\FloatBarrier

	\newpage\bigskip\large\textbf{5. Process Implementation}\bigskip\normalsize
...
\begin{table}[h]
	\begin{tcolorbox}[tabularx*={\arrayrulewidth0.6mm}{X|X|X|X|X},fontupper=\normalsize\sffamily,colback=white!10!white,colframe=black!40!]
		...
		{12} & {40} & {0.01} & {39.62\%} & {id 262}  \\\hline
		{12} & {40} & {0.001} & {38.23\%} & {id 260}  \\\hline
		{12} & {80} & {0.0001} & {39.96\%} & {id 261}  \\\hline
		{15} & {80} & {0.01} & {27.85\%} & {id 263}  \\\hline
		{15} & {80} & {0.0001} & {} & {}  \\\hline
	\end{tcolorbox}
\end{table} \FloatBarrier

9 and 80 are overfitting
12 and 80 starting to overfit as well
12 and 40 - testing stable, training decreasing 
15 and 80 and 0.01 - testing stable, training decreasing 
