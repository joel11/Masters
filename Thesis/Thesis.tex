...
\begin{technicaldoc}
	...
	\bigskip\large\textbf{1. Current Status}\bigskip\normalsize
	...
	\newpage
	\bigskip\large\textbf{2. Future Status}\bigskip\normalsize
	...
	\begin{itemize} 
		\item[$\bullet$] Database work: MongoDB/SQL implementation to record parameters and results 
		\item[$\bullet$] Learning Optimizations: Variable learning rate (AdaGrad/ADAM/ADA)
		\item[$\bullet$] Trading Strategy: Decide on the basic strategy used to derive returns	
		\item[$\bullet$] Synthetic Tests: Run tests on synthetic data to inform experiment design and paraeter choices
		\item[$\bullet$] Finalise Dataset: Need to properly account for survivorship bias, and decide which data to use
		\item[$\bullet$] SAE updates: Decide whether the SAE should be updated periodically or not
		\item[$\bullet$] Literature Review: There are various inclusions and ammendments to be made here
	\end{itemize}

	\paragraph{Non-Critical/Maybe}

	\begin{itemize}
		\item[$\bullet$] Better hyper-parameter search technique, e.g. Random Search
		\item[$\bullet$] Parallelization of training tasks
		\item[$\bullet$] Denoising SAEs
		\item[$\bullet$] Learning Optimization: Momentum
		\item[$\bullet$] Weight monitoring visualizations
	\end{itemize}	

	\newpage\bigskip\large\textbf{3. Milestones \& Deliverables}\normalsize
	...
	\begin{table}[h]
		\begin{tcolorbox}[tabularx*={\arrayrulewidth0.6mm}{X||X},fontupper=\normalsize\sffamily,colback=white!10!white,colframe=black!40!]
			...
			\vspace{0.1mm}  \centerline {Done} &\vspace{0.1mm}   {1 Literature Review}\\\hline
			\vspace{0.1mm}  \centerline {In Progress} &\vspace{0.1mm}   {2 Data Collection}  \\\hline
			\vspace{0.1mm}  \centerline {Done} &\vspace{0.1mm}   {3 Data Log Fluctuation Processing}  \\\hline
			\vspace{0.1mm}  \centerline {Done} &\vspace{0.1mm}   {4 SAE Implementation}  \\\hline
			\vspace{0.1mm}  \centerline {Done} &\vspace{0.1mm}   {5 Online FFN/LSTM Implementation} \\\hline
			\vspace{0.1mm}  \centerline {Done} &\vspace{0.1mm}   {6 CSCV Implementation} \\\hline		
			\vspace{0.1mm}  \centerline {Done} &\vspace{0.1mm}   {7 Synthetic Data Generation} \\\hline					
			\vspace{0.1mm}  \centerline {30/11} &\vspace{0.1mm}   {8 Finalise Implementation Process} \\\hline
			\vspace{0.1mm}  \centerline {30/11} &\vspace{0.1mm}   {9 Database Implementation} \\\hline
			\vspace{0.1mm}  \centerline {30/11} &\vspace{0.1mm}   {10 Learning Optimizations} \\\hline			
			\vspace{0.1mm}  \centerline {14/12} &\vspace{0.1mm}   {11 Test full implementation on synthetic
	data} \\\hline
			\vspace{0.1mm}  \centerline {14/01} &\vspace{0.1mm}   {12 Test full implementation on actual
dataset} \\\hline
			\vspace{0.1mm}  \centerline {31/01} &\vspace{0.1mm}   {13 Statistical Analysis (incl. profitability
of trading strategy)} \\\hline
			\vspace{0.1mm}  \centerline {28/02} &\vspace{0.1mm}   {14 Write up \& Revisions} \\\hline
									
		\end{tcolorbox}
	\end{table}
	\FloatBarrier

	\newpage\bigskip\large\textbf{4. Process Implementation}\bigskip\normalsize
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item[$\bullet$] The processed dataset is split into 3 parts, to be used for the following processes: SAE/SGD training, OGD training, hold-out validation (e.g. 60\%, 20\%, 20\%)
		\end{itemize}	
		\item [3] SAE Training
		\begin{itemize}
			...
			\item[$\bullet$] Once the SAE network has been defined, it can be used to reprocess the datasets such that the input is now encoded, and the output is as before. These encoded datasets will be used for all following steps.
		\end{itemize}		
		...
		\end{itemize}			
		\item [6] Holdout Validation
		\begin{itemize}
			\item[$\bullet$] The third and last portion of data will be used to obtain results for the model. Predictions will be made on the relevant stock price log fluctuations
		\end{itemize}				
		\item [7] Returns generations
		\begin{itemize}
	...
	\newpage\bigskip\large\textbf{5. Points of concern}\bigskip\normalsize
	
	There are several issues which are still undecided/uncertain or which could use further refinement:
	\begin{itemize}
		\item[$\bullet$] End to end process: Should the FFN network be trained with SGD prior to OGD, or should the first dataset just be used for OGD training on the FFN after pre-training/weight initialization has taken place?
		\item[$\bullet$] Predictions attempted: How many steps and stocks should be attempted in the prediction process?
		\item[$\bullet$] Trading strategy: The CSCV method requires returns to be used, which can then be assessed with something like the Sharpe Ratio. Should an actual trading strategy be used for each configuration here, or just a representation of accuracy which can be calculated as a single figure for each time period (such as MSE)? If a trading strategy is used, what should it be?
		\item[$\bullet$] SAE Updating: The current implementation does not have any SAE updates taking place as more data is observed. While easy to change, I'm not sure if or when the SAE network should be updated
		\item[$\bullet$] Denoising SAE: The initial proposal was to use denoising autoencoders. The implementation process is easy enough, but I'm unsure if denoising would be effective in a financial context (where data is already so noisy)		
		\item[$\bullet$] Final Dataset: The current dataset doesn't account for survivorship bias, and I'm a little unsure of how best to pick the final stocks/indices etc. (the intial proposal was rather limited in comparison to what is now available)
	\end{itemize}		

	\newpage\bigskip\large\textbf{6. Meeting Minutes}\bigskip\normalsize	
...
\newpage
\section{LiteratureReview}\label{LiteratureReview}
\subsection{Technical Analysis}\label{TechnicalAnalysis}
...
\subsection{Online Learning Algorithms and Gradient Descent} \label{OGD}
\hfill
...
\subsection{Backtesting and Model Validation} 
\hfill
...
The implementation focuses on bringing together several ideas: data reduction, deep learning with pre-training/weight initialization, online learning and backtest overfitting validation for the purposes of stock price prediction. To this end, the full process was implemented with the following steps:

\begin{itemize}
	\item [1] The dataset was split into 3 subsets: the SAE/FFN  portion, the OGD portion and the hold-out set for validation
	\item [2] The first subset was used to train the SAE and deep FFN using the SGD algorithm. These networks were implemented with pre-training and weight initialization techniques
	\item [3] The second subset of data was used to continue training the network in an online manner using OGD
	\item [4] The third and last 'hold-out' subset was used to generate log fluctuation prediction
	\item [5] The predictions from 5 were used in a simple trading strategy to generate returns
	\item [6] These returns were then used as an input for the CSCV process, and so obtain a PBO calculation for the implementation as a whole
\end{itemize}
...
\subsubsection{Greedy Layerwise SAE Training}
...
\subsection{Deep Network Weight Initializations}\label{imp_weights}

As noted in \ref{lr_trainingbackprop}, one of the primary barriers to effective deep learning networks was weight initializations: backpropagated gradient values from SGD would either vanish or explode after multiple layers if the initial weights were too small or too large. The effect of which was to either reduce or completely halt the networks learning process. The first breakthrough, as per \ref{DBN}, was to initialize the weights through greedy layerwise pretraining of Restricted Boltzmann Machines. Later research, as per \ref{}, has shown that weights can be initialized to maintain expected variance between the input and output layers. Both methods have been implemented and tested for the purposes of SAE and FFN training.



\subsubsection{Deep Belief Network Training}
