...
\begin{technicaldoc}
	...
	\begin{itemize} 
		\item[$\bullet$] Change SAE testing to use cross validation. The current (constant) split on training and testing data is resulting in significant overfitting issues.
		\item[$\bullet$] Training: Regularization methods \& Learning rate annealing schedule are implemented, but need to be tested and incorporated info configuration testing.
		\item[$\bullet$] Synthetic Tests: Run tests on synthetic data using the above features, and including Sigmoid activations with Weight Initialization \& Pre-Training techniques.
		\item[$\bullet$] Extended Training: The predictive models in this iteration were not trained for very long - they could probably be easily improved in this way.
	\end{itemize}
	
	\paragraph{Critical Decisions and Points of Concern}

	\begin{itemize} 
		\item[$\bullet$] Data Scaling - How should this work for different activations and online learning?
