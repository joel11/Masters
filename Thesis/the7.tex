...
%	successful \citep{Shalev, Zhang}. 
	...
	Several datasets have been used using JSE closing price relative data for 2003-2018 (more soruce information and a data snapshot are available in Appendix \ref{appendix_dataset}). They were processed following the steps set out in Section \ref{Data} and with a 60/40 split on the Training/Prediction datasets.
	...
	Further to the framework developed, a view on the phenomenology of financial markets has been presented based on the experiment results. The value of recent cross sectional data over historical data is shown through limited IS training of the networks (both by length of training and dataset size), the effects of feature reduction developed IS, as well as OOS learning configurations. These results emphasise the criticality of recent data in delivering out of sample returns. It's also shown that feature reduction for financial time series is possible and effective, despite the complexities present. In full, a framework has been presented and shown to not only deliver promising P\&L rates for out of sample data, but has also done so while being able to incur a low probability of backtest overfitting or spurious results otherwise.
	...
	The ``Actual10'' dataset (as per Section \ref{dataset_actual10}) was used for the testing and validation of the framework presented. It was produced by \citet{Gant} using Bloomberg data and consists of daily price relative share price data for the constituents of the JSE Top 40 (as of January 2003) from 2003 to 2018. A snapshot of the data can be seen in Figure \ref{figure-dataset_snapshot} below. \newline
