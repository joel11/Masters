...
%	successful \citep{Shalev, Zhang}. 
	...
	\begin{enumerate}
		...
		Using a cost function such as MSE (equation \ref{func_MSE}), the rate of change for the error in the output layer by the rate of change in the activation function in the last layer, $L$, is calculated. This rate of change for the error for the $j^{th}$ node, $\delta_j$, is expressed as:	
		...
		If using quadratic cost, as per equation \ref{func_MSE}, then the term $\frac{\partial C}{\partial a_{j}^{L}}$ can be reduced to $(a^L - y)$. The equation for the output layer error rate of change is then:
		...
		\begin{enumerate}
			...
			This uses the same rationale as equation \ref{eq_lambda1}, but rather uses $(w^{\ell+1})^T$ by the next layers error in order to get an indication of how the error rate and how it moves backwards through the network.
	...
	\begin{itemize}
		...
		\begin{enumerate}
			...
			\item $\delta$ is the rate of change in the error (calculated using equation \ref{eq_lambda4}), and
	...
	\end{enumerate}
	~\\
	It can be shown, that network weights can be adjusted to change the probabilities assigned to a particular training sample . The derivations of equation \ref{func_rbmenergy} show that performing a stochastic descent for the log probability of the data can be implemented through the following weight adjustment:
