...
\begin{document} 
	...
	experts will consider a company's operations, management and future prospects in order to arrive at an evaluation. 
	...
	values for an arbitrary time period \citep{Hochreiter}. Convolutional Neural Networks (CNNs) have a non-recurrent structure, 
	...
	effectively train deep layered networks. The critical suggestion by \citet{Hinton3} was to use layered restricted 
...
%	successful \citep{Shalev, Zhang}. 
	...
	All datasets are transformed into log feature fluctuation values, and are then aggregated to include fluctuations over rolling window periods. The log feature fluctuation for measured prices $\tilde{p}$ at timepoint $i$ is calculated as:
	...
	Once the log differenced data has been aggregated using equations \eqref{eq_price_agg_past} and \eqref{eq_price_agg_future}, the datasets are either standardized or normalized to allow for better learning. Typical implementations of these methods are detailed below, using the following notation:
	
	\begin{enumerate}
		\item $\mathbf{p}$ denotes a vector of log differenced price summations, as per equations \eqref{eq_price_agg_past} and \eqref{eq_price_agg_future},
		\item $\mathbf{\bar{p}}$ denotes the mean of any $\mathbf{p}$ vector,
		\item $\sigma_p$ denotes the standard deviation of any $\mathbf{p}$ vector, and
		\item $\mathbf{p}_{1:t}$ or $\sigma_{p_{1:t}}$ denote a subset of vector $\mathbf{p}$ from indices $1$ to $t$ and the standard deviation thereof, respectively.\newline
	\end{enumerate}
	...
	\begin{enumerate}
		\item A network $N$ will be constructed with $L$ layers, each with $e^l$ nodes,
		\item The weights from the $j^{th}$ node in the $(\ell-1)^{th}$ layer to the $k^{th}$ node in the $\ell^{th}$ layer are represented by $\mathbf{w}^\ell_{jk}$,
		...
		\mathbf{z^\ell_j}=\sum_{i=1}^{e^{\ell-1}}{\mathbf{a}^{\ell-1}_i\mathbf{w}^{l}_{ij}} + \mathbf{w}^{l}_{0j} .
		\end{equation}
	\end{enumerate}
	~\\
	...
	\begin{enumerate}
		...
		Equation \eqref{eq_weighted_input} is used to calculate the weighted output and the equations defined in Section \ref{imp_activation_functions} are used for the activation functions.
		\newline
	...
	\begin{enumerate}
		...
		\item $x$ is the dataset inputs,
		\item $y$ is the dataset outputs,
	...
	\begin{algorithm}[H]
		...
		\ForEach{$\mathbf{V^D}$ in $D$}
		{
			...
			\tcp{$\Delta \mathbf{w}_{ij} = \eta (\langle \mathbf{v}_i\mathbf{h}_j\rangle_{\mathrm{data}} - \langle \mathbf{v}_i\mathbf{h}_j\rangle_{\mathrm{reconstruction}})$}
	...
	\begin{longtable}{|p{0.25\linewidth}|p{0.375\linewidth}|p{0.375\linewidth}|}
		...
		{Transaction Capital Costs}
				&{The capital and transaction costs of buying the asset.}
...
%	That said, enjoy!
	...
	To this point, we found in Section \ref{results_data_hist} that the In Sample (IS) training on historical data had a limited benefit, and could become detrimental if trained on extensively. This was confirmed by showing the negligible impact of increasing training time as well as the surprisingly small impact of large reductions in the training data (see Figures \ref{figure-results_pl_max_epochs} and \ref{figure-results_it3_validationset}). It was made clear that the increased performance for IS data was not necessarily linked to Out of Sample (OOS) performance. This gives weight to the importance of online learning methods for financial applications, which in turn makes network initialization a significant configuration choice. Methodologies for effective initialization were explored in Section \ref{results_init}. We found positive results in the He-adj initialization presented in Section \ref{imp_weights} (see Figures \ref{figure-mse_init} and \ref{figure-pl_init}), as well as poor performance in Restricted Boltzmann Machine (RBM) based pretraining (see Figure \ref{figure-results-pretraining-effect}).  \newline
