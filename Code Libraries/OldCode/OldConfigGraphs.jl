...
function Results_4_NetworkStructureTraining()
    ...
    encoding_ffn_config_ids = SelectConfigIDs(["Iteration FFN Actual10 Tests"])
...
function Results_5_DataAggregation()
    ...
    OGD_ValidationSet_Profits_bx(large_ffn_config_ids)
    FFN_LR_Sched_BxProfit(large_ffn_config_ids)
    OGD_LR_BxProfit(large_ffn_config_ids)
    OGD_EpochCycle_BxProfit(large_ffn_config_ids)
    OGD_MaxEpochs_BxProfit(large_ffn_config_ids)
    OGD_Heatmap_LayerSizes_Epochs(large_ffn_config_ids)
end
...
function Results_7_FinancialResults()
    ...
    minimum(TotalProfits[:configuration_id])
    AllProfitsPDF(28880)
end

function PrintConfig(ids)
    sae_setnames = ["Iteration1 SAE Actual10 Test",
                "Iteration2 SAE Actual10 Test",
                "Iteration3 SAE Actual10 Test",
                "Iteration4 SAE Actual10 Test",
                ...
                "Iteration6 SAE Actual10 Test"]
    sae_config_ids = SelectConfigIDs(sae_setnames)
    ids = TransformConfigIDs(sae_config_ids)

    query = "select cr.configuration_id, tp.category, sae_config_id,
            ...
            tp.learning_rate, tp.minibatch_size, tp.max_epochs, tp.l1_lambda, ifnull(tp.l2_lambda,0), tp.min_learning_rate, tp.epoch_cycle_max, tp.is_denoising, tp.denoising_variance,
    ...
    for n in names(configs)
        ...
        config_string = string(config_string, n, ": ", vals, '\n')
    end
