...
#voxpoints = false, "all", "outliers", "suspectedoutliers"

function general_boxplot(layer_groups, xaxis_label, filename, variable_name, yaxis_label, xlabels_show, alternateColours = false)

    colors = alternateColours ? ColorBrewer.palette("PRGn", 11) : ColorBrewer.palette("Spectral", 11)
    ...
    l = Layout(width = 1000, height = 600, margin = Dict(:b => 100, :l => 100)
        ...
        , xaxis = Dict(:title => string("<b>", xaxis_label, " </b><br><br><br><i> Sample Sizes: ", string(group_sizes), "</i>"), :showticklabels => xlabels_show ))
...
function OGD_MaxEpochs_BxProfit(config_ids)
    ...
    ProfitBoxplot(query, :max_epochs, "PL Max Epochs", "PL Max Epochs", String, NullTransform)
end

function OGD_EpochCycle_BxProfit(config_ids)
    ...
    ProfitBoxplot(lr_query, :max_epochs, "PL Epoch Cycle", "PL Epoch Cycle", String, NullTransform)
end
...
function GenericHeatmap(results, filename)
    ...
    results = join(TotalProfits, results, on = :configuration_id)
    ...
    data = [trace]
    savefig(plot(data, l), filename)
end

function OGD_LearingRate_MaxEpochs(config_ids, filename)
    ...
    GenericHeatmap(results, filename)
end
