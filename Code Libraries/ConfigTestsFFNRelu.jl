...
#configs = mapreduce(x->string(x, ","), string, 504:604)[1:(end-1)]
#query = string("select * from prediction_results where configuration_id in ($configs)")
#@time results = RunQuery(query)
#=
i = 2
for i in 1:(length(config_ids))
    #c = map(i->i, config_ids[i]:(config_ids[i+1]-1))
    #println(c)
    c = config_ids[i]
    query = string("select * from prediction_results where configuration_id in ($c)")
    @time results = RunQuery(query)
    #groups = by(results, [:configuration_id, :stock], df -> [df])
    #groups = by(results, [:configuration_id], df -> [df])

    #profits = map(i -> sum(CalculateReturnsOneD(Array(groups[i,end][:,:actual]),
    #                                            Array(groups[i,end][:,:predicted]))), 1:size(groups,1))
