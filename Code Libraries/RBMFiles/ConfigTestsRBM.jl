...
function RunSAEPreTrainingTest(encoding_layer, layer_size, num_hidden)
    ...
    function GenerateBaseSAEConfig(set_name, datasetname)
        ...
        sae_net_par = NetworkParameters("SAE", layers, activations, InitializationFunctions.XavierGlorotNormalInit, SigmoidActivation, SigmoidActivation)
        sae_sgd_par = TrainingParameters("SAE", 2.0, 1.0, 1, 20, 0.0, 100, (0.0001, 100), NonStopping, true, false, 0.0, 0.0, MeanSquaredError(), [0.8])
    ...
    push!(vps, (GetSAETraining, ChangeMaxLearningRate, (1.0, 2.0, 4.0)))
    push!(vps, (GetRBMTraining, ChangeMaxLearningRate, (0.1, 0.5, 1.0, 2.0)))
    push!(vps, (GetRBMTraining, ChangeMaxEpochs, (0, 1, 10, 50)))
    push!(vps, (GetSAENetwork, ChangeEncodingActivation, (LinearActivation, SigmoidActivation)))

    set_name =  string("PT 3 SAE Sigmoid Test Set ", num_hidden, "x", layer_size, "x", encoding_layer)
