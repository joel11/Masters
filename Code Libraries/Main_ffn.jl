...
function GenerateBaseExperimentConfig()
    ...
    sae_net_par = NetworkParameters("SAE", [input_size, 8,  encoding_layer],[ReluActivation,  LinearActivation], InitializationFunctions.XavierGlorotNormalInit)
    #sae_net_par = NetworkParameters("SAE", [input_size, 8, 8, encoding_layer],[SigmoidActivation, SigmoidActivation, LinearActivation], InitializationFunctions.XavierGlorotNormalInit)
    ...
    return ExperimentConfig(seed, "Null Name", rbm_pretraining, data_config, sae_net_par, ffn_net_par, sae_sgd_par, ffn_sgd_par, rbm_cd, ogd_par, false)#, holdout_ogd_par)
end
...
sae_results = map(ep -> RunSAEConfigurationTest(ep), combos)

using ExperimentGraphs
PlotSAERecontructions(sae_results, "sae_recons")
